...
class preamble_optimization(Problem):
    ...
    def flat_coeffs_to_4D(self, coeff_flat_arr):
        ...
        # J x M x 2K arrays of coefficients for all signal sets within generation, J = # signals per generation
    ...
    def _evaluate(self, coeff_flat_arr, out, *args, **kwargs):
        ...
        for j in range(len(generation_all_coeffs[0])):
            ...
            self.data[self._gen_num].append([max_max_crosscorr])

            # gen_results.append([max_ISL, max_max_crosscorr])
            gen_results.append([max_max_crosscorr])
        print(self._gen_num)
        self._gen_num += 1
