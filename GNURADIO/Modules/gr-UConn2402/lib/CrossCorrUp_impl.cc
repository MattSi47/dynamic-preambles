...
namespace gr {
  namespace UConn2402 {
   ...
   // #pragma message("set the following appropriately and remove this warning")
    ...
    CrossCorrUp::make()
    {
      return gnuradio::make_block_sptr<CrossCorrUp_impl>(
        );
    }
     ...
     */
    CrossCorrUp_impl::CrossCorrUp_impl()
      : gr::block("CrossCorrUp",
    ...
    float CrossCorrUp_impl::CrossCorr(const gr_complex* input, gr_complex* pattern)
{
    ...
    void CrossCorrUp_impl::forecast (int noutput_items, gr_vector_int &ninput_items_required)
    {
      ninput_items_required[0] = noutput_items + 160;
    }

    int
    CrossCorrUp_impl::general_work (int noutput_items,
