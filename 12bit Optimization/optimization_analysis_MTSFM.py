...
import datetime
os.chdir(sys.path[0])
from optimizationV3_MTSFM_12bit import preamble_optimization
...
M = 6
N = 160
timestamp = "2024-03-22_03-32-17"  # Replace with the timestamp from your files
pareto_filename = f"opt_signals/pareto-signals_M{M}-N{N}_{timestamp}.npy"
optimization_filename = f"opt_signals/optimization-data_{timestamp}.npy"
dummy_opt = preamble_optimization(M, N)

# Load the files
pareto_coeffs = np.load(pareto_filename)
pareto_coeffs_formatted = dummy_opt.flat_coeffs_to_4D(pareto_coeffs)
norm_pareto_set = [dummy_opt.gen_waveforms(coeff_set[0], coeff_set[1]) for coeff_set in pareto_coeffs_formatted]
...
pareto_data = [(dummy_opt.ISL_obj_fn(Si), dummy_opt.crosscorr_obj_fn(Si)) for Si in norm_pareto_set]
# ====================================================== #
